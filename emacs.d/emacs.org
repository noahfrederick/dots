...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package auto-complete-config
    ...
    (progn
      ...
      (define-key evil-insert-state-map (kbd "C-l") 'ac-complete-with-helm)
      (define-key ac-complete-mode-map (kbd "C-l") 'ac-complete-with-helm)
  ...
  Ledger expects. Includes a custom prompt string."
        ...
        (ido-completing-read "Account: "
                             (split-string
                              (with-output-to-string
                                (shell-command "ledger accounts" standard-output))
                              "\n" t)))
      ...
      (defun nox/ledger-org-read-payee ()
        ...
        (ido-completing-read "Payee: "
                             (split-string
                              (with-output-to-string
                                (shell-command "ledger payees" standard-output))
                              "\n" t)))
      ...
      (setq org-capture-templates
            '(("t" "☑️ To-do")
              ...
              ("ll" "👖 Laundry" plain
               ...
               :clock-keep t :empty-lines-before 1 :jump-to-captured t)
              ("lI" "💸 Transfer to IRA" plain
               (file "~/ledger/default.ledger")
               (file "~/ledger/xacts/ira.txt")
               :clock-keep t :empty-lines-before 1 :jump-to-captured t :immediate-finish t)
              ("lD" "🕐 Monthly Dividends" plain
               (file "~/ledger/default.ledger")
               (file "~/ledger/xacts/dividends_monthly.txt")
               :clock-keep t :empty-lines-before 1 :jump-to-captured t :immediate-finish t)
              ("lE" "🕒 Quarterly Dividends" plain
               (file "~/ledger/default.ledger")
...
[[https://github.com/sabof/org-bullets][org-bullets]] changes the appearance of the character preceding
headings by cycling through the characters in ~org-bullets-bullet-list~.
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package mu4e
    ...
    (progn
      ...
      (defvar nox/mu4e-new-mail nil "Whether there is any new mail.")

      (setq global-mode-string
            (if (string-match-p "nox/mu4e-new-mail"
      ...
      (defun nox/mu4e-new-mail-count ()
        "Get number of new messages from mu4e."
        (string-to-number
         (replace-regexp-in-string
          "[ \t\n\r]" "" (shell-command-to-string
                          "mu find flag:new | wc -l"))))

      (defun nox/mu4e-index-updated ()
        (when (fboundp 'sauron-add-event)
          (let ((msgs (nox/mu4e-new-mail-count)))
            (if (> msgs 0)
                (progn
                  (sauron-add-event 'mail 5 (format "%s new message(s)" msgs))
                  (setq nox/mu4e-new-mail t))
              (sauron-add-event 'mail 1 "Mail index updated")
              (setq nox/mu4e-new-mail nil)))))

      (add-hook 'mu4e-index-updated-hook #'nox/mu4e-index-updated)
